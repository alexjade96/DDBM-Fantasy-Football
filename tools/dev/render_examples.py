...
"""Regenerate every analytics graphic + summary from the Python package.
    ...
    fantasy-football-4-fun/venv/Scripts/python tools/dev/render_examples.py [league_id]

Champions come from the stored playoff brackets (see data/seasons/README.md),
not Sleeper's winners_bracket -- so crowns, titles and champion stars are
correct. Output: r-analysis/data/results/examples/py/ (gitignored build
artifacts).
"""
...
def main():
    ...
    ss = sm.apply_playoffs(sm.seasons(league), str(SEASON_DIR))
    latest = list(ss.values())[-1]
    pos = sm.load_playoffs(str(SEASON_DIR))
