...
Run from repo base via the python venv:
    fantasy-football-4-fun/venv/Scripts/python tools/parity/export_py.py <league_id> tools/parity/out_py.json

The R exporter (tools/parity/export_r.R) emits the identical structure so
verify.py can assert the two implementations mirror each other.
"""
import json
import sys

from paths import PY_PACKAGE_DIR, SEASON_DIR

sys.path.insert(0, PY_PACKAGE_DIR)  # import the python-instance package
...
def main():
    ...
    out_path = sys.argv[2] if len(sys.argv) > 2 else "tools/parity/out_py.json"
    ...
    ss = sm.apply_playoffs(ss, SEASON_DIR)
    pos = sm.load_playoffs(SEASON_DIR)
    ...
    out = {
        ...
        "allplay": recs(metrics.allplay(latest).sort_values("user_name"),
                        ["user_name", "allplay_w", "allplay_l", "allplay_pct",
                         "final_position", "allplay_rank", "rank_delta"]),
        "power_rank": recs(metrics.power_rank(latest).sort_values("user_name"),
                           ["user_name", "points", "allplay_pct", "form", "eff",
                            "power", "power_rank"]),
        "manager_profile": recs(metrics.manager_profile(latest).sort_values("user_name"),
                                ["user_name", "moves", "trades", "drops",
                                 "moves_per_wk", "lineup_iq"]),
