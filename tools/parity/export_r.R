...
# Run from repo base:
#   Rscript tools/parity/export_r.R <league_id> tools/parity/out_r.json
# Emits the identical structure to tools/parity/export_py.py so verify.py can
# assert the two implementations mirror each other.
#
# Keep the package path and season dir below in sync BY HAND with
# tools/parity/paths.py's PY_PACKAGE_DIR/SEASON_DIR -- R has no portable
# "this file's own directory" primitive to centralize this the way the
# Python side does, so this comment is the cross-reference.

suppressWarnings(suppressMessages({
  pkgload::load_all("r-analysis/sleepermetrics", quiet = TRUE)
  library(dplyr); library(jsonlite)
}))

args <- commandArgs(trailingOnly = TRUE)
league <- if (length(args) >= 1) args[[1]] else "1252770181306929152"
out_path <- if (length(args) >= 2) args[[2]] else "tools/parity/out_r.json"
...
ss <- sl_apply_playoffs(ss, sl_season_dir())
pos <- sl_load_playoffs(sl_season_dir())
latest <- ss[[length(ss)]]
...
out <- list(
  ...
  allplay = recs(sl_allplay(latest),
                 c("user_name", "allplay_w", "allplay_l", "allplay_pct",
                   "final_position", "allplay_rank", "rank_delta"), "user_name"),
  power_rank = recs(sl_power_rank(latest),
                    c("user_name", "points", "allplay_pct", "form", "eff",
                      "power", "power_rank"), "user_name"),
  manager_profile = recs(sl_manager_profile(latest),
                         c("user_name", "moves", "trades", "drops",
                           "moves_per_wk", "lineup_iq"), "user_name"),
  ...
  trade_performance = recs(sl_trade_performance(latest),
                           c("player_id", "player_name", "user_name", "weeks",
                             "points", "avg", "total"),
                           "player_name"),
  ...
  playoff_players = recs(head(sl_playoff_players(pos), 25),
                         c("player_id", "player_name", "position", "seasons",
                           "games", "points", "ppg", "best", "rings"), "player_name"),
