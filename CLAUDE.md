...
- **`sleepermetrics/`** — a proper R package (the general, canonical implementation). Roles are segmented into files: `api.R` (`sleeper_api`), `players.R`, `league.R`, `season.R` (assembles a `sleeper_season` S3 object + the roster-aware optimal-lineup solver), `metrics.R` (compute → tibbles), `plots.R` (render → ggplot), `summaries.R` (narrate → markdown), `dashboard.R`. Public API is prefixed `sl_` (e.g. `sl_season()`, `sl_luck()`, `sl_plot_efficiency()`, `sl_summary_season()`, `sl_dashboard()`). Has roxygen docs, a `NAMESPACE`, and network-free `testthat` tests. Load with `pkgload::load_all("sleepermetrics")`.
