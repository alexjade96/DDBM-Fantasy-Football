...
- **`ddbmFF.R`** — the **origin source**: single-season analytics (one season's matchups/rosters/transactions → ~26 charts in `results/<season>/`). Run interactively in RStudio (`FantasyFootball.Rproj`) or headless via `Rscript`. Kept as-is as the reference implementation.
...
- **`sleepermetrics/`** — a proper R package (the general, canonical implementation). Roles are segmented into files: `api.R` (`sleeper_api`), `players.R`, `league.R`, `season.R` (assembles a `sleeper_season` S3 object + the roster-aware optimal-lineup solver), `metrics.R` + `weekly.R` (compute → tibbles), `plots.R` (render → ggplot), `summaries.R` (narrate → markdown), `dashboard.R` (Shiny), `discord.R` (Discord). Public API is prefixed `sl_` (e.g. `sl_season()`, `sl_luck()`, `sl_plot_efficiency()`, `sl_summary_season()`, `sl_dashboard()`). Has roxygen docs, a `NAMESPACE`, and network-free `testthat` tests. Load with `pkgload::load_all("sleepermetrics")`.
...
- **`app.R` + `R/ddbmMetrics.R`** — the **prototype** Shiny dashboard the package was generalised from (still runnable). The package's bundled app (`sleepermetrics/inst/shinyapp/app.R`, launched via `sl_dashboard()`) supersedes it; both hit the same metrics. The optimal-lineup solver in both reads each league's `roster_positions`, so FLEX/SUPER_FLEX/etc. adapt per league.
