...
#' @param playoffs Directory of playoff bracket configs to expose in the
#'   Playoffs tab. Defaults to `playoffs/` under the current working directory
#'   if it exists. Resolved to an absolute path *before* the app starts, because
#'   [shiny::runApp()] moves the working directory to the app folder.
...
#' }
#' @export
sl_dashboard <- function(league_id = NULL, playoffs = "playoffs", ...) {
  ...
  if (!is.null(playoffs) && dir.exists(playoffs)) {
    Sys.setenv(SLEEPERMETRICS_PLAYOFFS = normalizePath(playoffs))
  }
