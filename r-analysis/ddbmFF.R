...
# Path resolution (lets this script find its own directory regardless of
# how/where it's invoked from -- see the SCRIPT_DIR block below):
...
# This script's own directory, so its file I/O (the player cache, the
# per-season results/ output) works the same way regardless of invocation
# style: sourced in RStudio via r-analysis/FantasyFootball.Rproj, run
# headless as `Rscript r-analysis/ddbmFF.R` from the repo root, or run as
# `Rscript ddbmFF.R` from inside r-analysis/ itself. `this.path()` resolves
# correctly in all three; `dirname()` of an interactively-sourced script
# still returns the file's real directory.
...
# Per-season output directory for charts and logs (data/results/<season>/).
# out("DDBMx.png") builds a season-namespaced path so seasons never overwrite.
outDir <- file.path(SCRIPT_DIR, "data", "results", season)
...
playerFilename <- file.path(SCRIPT_DIR, "sleeperPlayerData.rds")
