...
#'
#' @param path Path to a playoff JSON config (see `data/seasons/<league_id>/` in the repo).
...
#'
#' @param playoff_dir Root directory of bracket configs (default:
#'   [sl_season_dir()]'s own default).
#' @param league_ids Optional character vector of league ids to keep.
#' @return Named character vector: `season -> config path`.
#' @export
sl_playoff_configs <- function(playoff_dir = sl_season_dir(), league_ids = NULL) {
...
#' @export
sl_apply_playoffs <- function(seasons, playoff_dir = sl_season_dir(), recompute = FALSE) {
...
#' @export
sl_load_playoffs <- function(playoff_dir = sl_season_dir(), league_ids = NULL) {
  ...
  dplyr::bind_rows(lapply(names(playoffs), function(s) {
    ...
    dplyr::bind_rows(lapply(unique(r$matchup_id), function(m) {
      ...
      opt <- vapply(g$team, function(tm) {
        ...
        # nothing here (mirrors pl_wk_all in fantasy-football-4-fun/sleepermetrics/playoffs.py).
