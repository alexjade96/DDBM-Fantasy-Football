...
#'   `<playoffs>/adp/`) to expose in the Playoffs tab. Defaults to
#'   [sl_season_dir()]'s own default (`data/seasons/` resolved relative to
#'   the repo root, or `SLEEPERMETRICS_SEASON_DIR` if set) if that path
#'   exists. Resolved to an absolute path *before* the app starts, because
...
#' }
#' @export
sl_dashboard <- function(league_id = NULL, playoffs = sl_season_dir(), ...) {
