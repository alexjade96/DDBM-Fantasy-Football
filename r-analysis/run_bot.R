...
#   Rscript r-analysis/run_bot.R serve                 # interactions endpoint (plumber)
#   Rscript r-analysis/run_bot.R weekly [--dry-run]    # weekly recap poster
# Config: r-analysis/.env (auto-loaded if present) or the environment.

args <- commandArgs(trailingOnly = TRUE)
mode <- if (length(args) >= 1) args[[1]] else "serve"
dry  <- any(args == "--dry-run")

if (file.exists("r-analysis/.env")) readRenviron("r-analysis/.env")

if (requireNamespace("sleepermetrics", quietly = TRUE)) {
  suppressMessages(library(sleepermetrics))
} else {
  suppressMessages(pkgload::load_all("r-analysis/sleepermetrics", quiet = TRUE))
}
...
if (mode == "weekly") {
  ...
  if (!dry && !nzchar(webhook))
    stop("Set DISCORD_WEBHOOK in r-analysis/.env (or pass --dry-run).")
