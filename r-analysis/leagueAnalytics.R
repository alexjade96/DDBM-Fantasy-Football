...
#
# Output: charts + summary written to data/results/league/.
# Run headless: Rscript leagueAnalytics.R (from r-analysis/) or
#   Rscript r-analysis/leagueAnalytics.R (from the repo root) -- either works,
#   the script locates its own directory via this.path (see SCRIPT_DIR below).
# Or source in RStudio via r-analysis/FantasyFootball.Rproj.
...
# Config #
##########
# This script's own directory -- see ddbmFF.R's SCRIPT_DIR comment for why.
SCRIPT_DIR <- dirname(this.path::this.path())

currentLeagueId <- "1252770181306929152"
sortPosition <- c("QB", "RB", "WR", "TE", "K", "DEF")
outDir <- file.path(SCRIPT_DIR, "data", "results", "league")
...
playerDF <- readRDS(file.path(SCRIPT_DIR, "sleeperPlayerData.rds"))
