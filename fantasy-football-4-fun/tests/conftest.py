...
def _block_network(monkeypatch):
    """Make the suite's network-freedom an enforced invariant, not an accident.

    Every real external call in this codebase (Sleeper, ESPN, Yahoo, CBS, FFC,
    RotoWire, nflverse) goes through a fetch function every test monkeypatches
    at its own module boundary. That made the suite network-free in practice,
    but nothing stopped a provider's `except Exception` (see ffadp/nflref
    fetch()) from swallowing a misconfigured monkeypatch and silently falling
    through to a stubbed cache -- a broken "don't hit the network" test would
    still pass. Blocking the socket itself means a stray live call now raises
    loudly from inside whatever broad except wraps it, instead of vanishing.
    """
    def _blocked(*a, **k):
